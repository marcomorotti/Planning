...
Response = MsgBox("Vuoi calcolare gli ARTICOLI OBSOLETI MAI MOVIMENATI NEL PERIODO? ", vbYesNo, _
    "Continue")
...
If vbYes = MsgBox("Vuoi esportare i dati in  " & filenm, _
    ...
    Set bqry = db0.QueryDefs("qryArticoliObsoletiMaiMovimentati")
    ...
    MsgBox ("I Dati sono stati salvati come TSV file" & CurrentProject.Path & _
        "\Export\" & Format(Date, "yyyymmdd") & "ArticoliObsoletiMaiMovimentati_" & i & ".xls")
    brs.Close
    Set brs = Nothing
    Set db0 = Nothing
    Close #intFile
