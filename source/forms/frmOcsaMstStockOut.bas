        ...
        Begin Section
            ...
            Begin
                Begin Subform
                    ...
                    Width =17385
                    Height =4995
                    Name ="fsubOcsamstStockOut"
                    SourceObject ="Form.fsubOcsamstStockOut"
                ...
                Begin Subform
                    ...
                    Height =2250
                    TabIndex =2
                    Name ="fsubOcsaMstPOStockOut"
                    SourceObject ="Form.fsubOcsaMstPOStockOut"
                ...
                Begin ComboBox
                    ...
                    RowSource ="SELECT  qryCordersStockOut.DS_RAG_SOC FROM qryCordersStockOut WHERE (((qryCorder"
                        "sStockOut.Utente) like [forms]![frmOcsaMstStockOut]![txtUtente] & \"*\"))  GROUP"
                        " BY qryCordersStockOut.DS_RAG_SOC UNION select nullid as DS_RAG_SOC  from tblDum"
                        "my ORDER BY qryCordersStockOut.DS_RAG_SOC;"
                    ColumnWidths ="2886"
                    AfterUpdate ="[Event Procedure]"
                    OnGotFocus ="[Event Procedure]"

                    LayoutCachedLeft =11900
                    LayoutCachedTop =118
                    LayoutCachedWidth =15020
                    LayoutCachedHeight =358
                End
                Begin CommandButton
                    OverlapFlags =85
                    Left =15846
                    ...
                    ObjectPalette = Begin
                        0x000301000000000000000000
                    End
                    ControlTipText ="Reset Filtro"

                    LayoutCachedLeft =15846
                    LayoutCachedWidth =16278
                ...
                Begin ComboBox
                    ...
                    Name ="txtNumero_Doc"
                    RowSourceType ="Table/Query"
                    RowSource ="SELECT  Numero_Doc FROM qryCordersStockOut WHERE (((qryCordersStockOut.Utente) l"
                        "ike [forms]![frmOcsaMstStockOut]![txtUtente] & \"*\" ))  GROUP BY qryCordersStoc"
                        "kOut.Numero_Doc UNION select nullid as Numero_Doc  from tblDummy ORDER BY qryCor"
                        "dersStockOut.Numero_Doc;"
                    ColumnWidths ="2886"
                    AfterUpdate ="[Event Procedure]"
                    OnGotFocus ="[Event Procedure]"

                    LayoutCachedLeft =8420
                    LayoutCachedTop =120
                ...
                Begin ComboBox
                    ...
                    Name ="txtUtente"
                    RowSourceType ="Table/Query"
                    RowSource ="SELECT tblCOrders.Utente FROM tblCOrders GROUP BY tblCOrders.Utente ORDER BY tbl"
                        "COrders.Utente UNION select \"\" as Utente   from tblDummy;"
                    ColumnWidths ="2886"
                    AfterUpdate ="[Event Procedure]"
...
On Error GoTo Err_cmdOpenMember_Click
    ...
    stLinkCriteria = "[COD_ART] = '" & Forms!frmOcsamstStockOut!fsubOcsamstStockOut.Form.txtCod_Art & "'"
...
Dim rst As DAO.Recordset
 Dim Varwhere As Variant
    ...
    Varwhere = Null
    ' Do Utente
    If Not IsNothing(Me!txtUtente) Then
      Varwhere = (Varwhere + " AND ") & "[Utente] LIKE '" & Me.txtUtente & "*'"
      End If
    ' Do Numero_Doc
    If Not IsNothing(Me.txtNumero_Doc) Then
        Varwhere = (Varwhere + " AND ") & "[Numero_Doc] LIKE '" & Me.txtNumero_Doc & "*'"
        End If
    ' Do Rag_Soc next
    If Not IsNothing(Me.txtRag_Soc) Then
        Varwhere = (Varwhere + " AND ") & "[DS_Rag_Soc] LIKE '" & Me.txtRag_Soc & "*'"
    End If
    If IsNothing(Varwhere) Then
        ' Open a recordset to see if any rows returned with this filter
     ' Cerca se trova niente
        Set rst = DBEngine(0)(0).OpenRecordset("SELECT * FROM qryCOrdersStockOutExe")
        If rst.RecordCount = 0 Then
    ...
    Else
     ' Open a recordset to see if any rows returned with this filter
     ' Cerca se trova niente
        Set rst = DBEngine(0)(0).OpenRecordset("SELECT * FROM qryCOrdersStockOutExe WHERE " & Varwhere)
        If rst.RecordCount = 0 Then
            MsgBox "Non ho trovato nessun ordine.", vbInformation, gstrAppTitle
            ' Clean up recordset
            rst.Close
...
q = False
Call GenerateCsvOrdersStockOut(Varwhere, q)
Exit Sub
End Sub
...
Dim rst As DAO.Recordset
 Dim Varwhere As Variant
    ...
    If IsNothing(Varwhere) Then
        ' Open a recordset to see if any rows returned with this filter
     ' Cerca se trova niente
        Set rst = DBEngine(0)(0).OpenRecordset("SELECT * FROM qryCOrdersExe")
        If rst.RecordCount = 0 Then
            MsgBox "Non ho trovato nessun ordine.", vbInformation, gstrAppTitle
            ' Clean up recordset
            rst.Close
    ...
    Else
     ' Open a recordset to see if any rows returned with this filter
     ' Cerca se trova niente
        Set rst = DBEngine(0)(0).OpenRecordset("SELECT * FROM qryCOrdersExe WHERE " & Varwhere)
        If rst.RecordCount = 0 Then
            MsgBox "Non ho trovato nessun ordine.", vbInformation, gstrAppTitle
            ' Clean up recordset
            rst.Close
...
q = False
Call GenerateExcelOrders(Varwhere, q)
Exit Sub
End Sub
...
Dim curForm As Form
    'Set the curForm variable to the currenly active form.
...
'    Me!txtUtente.Enabled = True
'    Me!txtNumero_Doc.Enabled = False
'    Me!txtRag_Soc.Enabled = False
    Me.txtRag_Soc = Null
    Me.txtNumero_Doc = Null
    Me.txtUtente = Null
    [Forms]![frmOcsamstStockOut]![fsubOcsamstStockOut].Requery
End Sub
...
Private Sub txtNumero_Doc_GotFocus()
'If txtUtente = 0 Or IsNull(txtUtente) Then
'    MsgBox "Specifica prima l'Utente"
'    txtUtente.SetFocus
'    Me!txtNumero_Doc.Enabled = False
'Else
    txtNumero_Doc.Requery
'End If
End Sub

Private Sub txtRag_Soc_AfterUpdate()
    [Forms]![frmOcsamstStockOut]![fsubOcsamstStockOut].Requery
'    Me!txtNumero_Doc.Enabled = False
'    Me!txtNumero_Doc = Null
'If Not IsNull(txtNumero_Doc) Then
'    Me!txtNumero_Doc.Enabled = True
'    Me!txtNumero_Doc.Enabled = False
'End If
End Sub

Private Sub txtRag_Soc_GotFocus()
'If txtUtente = 0 Or IsNull(txtUtente) Then
'    MsgBox "Specifica prima l'Utente"
'    txtUtente.SetFocus
'    Me!txtRag_Soc.Enabled = False
...
Private Sub txtUtente_AfterUpdate()
    [Forms]![frmOcsamstStockOut]![fsubOcsamstStockOut].Requery
'    Me!txtNumero_Doc.Enabled = False
'    Me!txtNumero_Doc = Null
'    Me!txtRag_Soc = Null
'If Not IsNull(txtUtente) Then
'    Me!txtNumero_Doc.Enabled = True
'    Me!txtRag_Soc.Enabled = True
'End If
End Sub
