...
Response = MsgBox("Vuoi calcolare IL COSTO ANNUALE DEGLI ARTICOLI ? ", vbYesNo, _
    "Continue")
...
If vbYes = MsgBox("Vuoi esportare i dati in  " & filenm, _
    ...
    ' Inserisce testata
    strDataLine = "Codice" & Chr(9) & "Descrizione" & Chr(9) & "Stato" & Chr(9) & "RopAct" & Chr(9) _
        & "RopProp" & Chr(9) & "ScortaSicurezzaForzata" & Chr(9) & "RoqAct" & Chr(9) & "RoqProp" & Chr(9) _
        & "TcaAct" & Chr(9) & "TcaProp" & Chr(9) & "ClasseMovi" & Chr(9) & "Giacenza" & Chr(9) & "ConsAnnuo" & Chr(9) _
        & "ABCGiacenza" & Chr(9) & "ABCConsumo" & Chr(9) & "ABCConsumoLs" & Chr(9) & "CsCst"
    Print #intFile, strDataLine
    'Ora loop per il recordset e scrive un TSV file per ogni record
        
    
    Set bqry = db0.QueryDefs("qryTca")
    Set brs = bqry.OpenRecordset
    
    Do While Not brs.EOF
    strDataLine = brs.Fields("Cod_art").Value & Chr(9) & brs.Fields("Des_art").Value & Chr(9) & _
        brs.Fields("Stato").Value & Chr(9) & brs.Fields("RopAct").Value & Chr(9) & _
        brs.Fields("RopProp").Value & Chr(9) & brs.Fields("ScortaSicurezzaForzata").Value & Chr(9) & _
        brs.Fields("RoqAct").Value & Chr(9) & brs.Fields("RoqProp").Value & Chr(9) & _
        brs.Fields("TcaAct").Value & Chr(9) & brs.Fields("TcaProp").Value & Chr(9) & _
        brs.Fields("Classe_Movi").Value & Chr(9) & brs.Fields("Giacenza").Value & Chr(9) & _
        brs.Fields("ConsAnnuo").Value & Chr(9) & brs.Fields("ABCGiacenza").Value & Chr(9) & _
        brs.Fields("ABCConsumo").Value & Chr(9) & brs.Fields("AbcConsumoValoreLs").Value & Chr(9) & _
        brs.Fields("Cs_Csc").Value & Chr(9)
    Print #intFile, strDataLine
    brs.MoveNext
    strDataLine = ""
    Loop
    MsgBox ("I Dati sono stati salvati come TSV file" & CurrentProject.Path & _
        "\Export\" & Format(Date, "yyyymmdd") & "CostoAnnualeArticoli_ " & i & ".xls")
    brs.Close
    Set brs = Nothing
    Set db0 = Nothing
    Close #intFile
